angular.module('tdttnoModule',[])
	.directive('ngTdttno',[function(){
	return {
		restrict:'E',
		scope:{
			ngData:'=',
		...
		templateUrl:"templates/vouchers/tdttno/templates/table.html",
		controller:['$scope','$modal','dmkh','dmvat','$timeout','$http',function($scope,$modal,dmkh,dmvat,$timeout,$http){
			...
			$scope.getInvoice = function(){
				...
				var url =server_url + "/api/" + id_app + "/getinvoice2receive?ma_kh=" + $scope.ngMasterData.ma_kh;
				...
				$http.get(url).success(function(invoices){
					...
					if($scope.ngData.length==0){
						alert("Không có hóa đơn nào cần thu cho khách hàng này");
					}
			...
			$scope.openDetail = function (size,template) {
				var modalInstance = $modal.open({
				  templateUrl:"templates/vouchers/tdttno/templates/edit.html",
