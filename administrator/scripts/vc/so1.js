...
so1Module.module.directive("saleOrder",function(){
	return {
		...
		controller:['$scope','so1','$rootScope','$interval','$location','appInfo',function($scope,so1,$rootScope,$interval,$location,appInfo){
			appInfo.info("so1",function(error,uerinfo,appinfo){
				...
				$scope.editOrder = function(_id){
					$location.url("/so1/edit/" + _id);
				}
				$scope.condition ={trang_thai:{$in:['1','2']}};
				$scope.filter = function(c){
					$rootScope.nextTick(function(){
						if(c){
						 _.extend($scope.condition,c);	
						}
						so1.load(id_app,{condition:$scope.condition,limit:10}).success(function(orders){
							$scope.orders = orders;
						})
					})
